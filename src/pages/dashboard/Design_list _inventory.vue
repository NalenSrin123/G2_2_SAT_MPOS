...
// Controls inline editing mode state on the same page
...
// Enable inline editing mode inputs
const startRestock = () => {
...
// Cancel and discard typed modifications
const cancelRestock = () => {
  isEditing.value = false
  tempStocks.value = {}
}

// Save inputs, refresh status levels, and update dashboard card metrics
const saveRestock = () => {
  let rawRestockedCount = 0

  items.value.forEach(item => {
    const updatedValue = parseInt(tempStocks.value[item.sku]) || 0
    if (updatedValue !== item.stock) {
      ...
      // Update historical reference flag to today
      item.restocked = new Date().toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' })

      // Dynamic rule recalculation 
      if (item.stock <= 3) {
        item.status = 'Critical'
        item.statusClass = 'border border-[#ffd7d7] bg-[#fff1f1] text-[#d11111]'
      } else if (item.stock < 20) {
        item.status = 'Low Stock'
        item.statusClass = 'border border-[#ffd7d7] bg-[#fff1f1] text-[#d11111]'
      } else {
        item.status = 'Optimal'
        item.statusClass = 'bg-[#dff8e9] text-[#14994c]'
      }
    }
  })

  // Synchronize layout card alerts counter
  const newLowStockCount = items.value.filter(i => i.stock < 20).length
  stats.value[1].value = `${newLowStockCount} Items`

  if (rawRestockedCount > 0) {
    stats.value[2].value = `${242 + rawRestockedCount} Units`
...
<template>
  <main class="min-h-screen bg-[#f7f8fb] px-4 py-5 text-[#22252c] sm:px-7 sm:py-6.5">
    ...
    <section class="mb-7 flex flex-col items-start justify-between gap-5 min-[821px]:flex-row min-[821px]:items-center">
      ...
      <div class="grid w-full grid-cols-1 gap-3 min-[560px]:grid-cols-2 min-[821px]:flex min-[821px]:w-auto min-[821px]:items-center min-[821px]:gap-2.5">
        ...
        <template v-else>
          <button
            @click="cancelRestock"
            class="inline-flex h-12 w-full cursor-pointer items-center justify-center gap-2.5 whitespace-nowrap rounded-md border border-[#e1e5ec] bg-white px-4 text-sm font-bold text-[#475467] transition duration-200 min-[821px]:h-11 min-[821px]:w-auto min-[821px]:px-4.5"
            type="button"
          >
            <span>Cancel</span>
          </button>
          <button
            @click="saveRestock"
            class="inline-flex h-12 w-full cursor-pointer items-center justify-center gap-2.5 whitespace-nowrap rounded-md border-0 bg-[#14994c] px-4 text-sm font-bold text-white shadow-[0_8px_18px_rgba(20,153,76,0.24)] transition duration-200 hover:-translate-y-px min-[821px]:h-11 min-[821px]:w-auto min-[821px]:px-4.5"
            type="button"
          >
            <svg class="h-5 w-5 flex-none fill-none stroke-current stroke-2 [stroke-linecap:round] [stroke-linejoin:round]" viewBox="0 0 24 24" aria-hidden="true">
    ...
    <section class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3 lg:gap-6" aria-label="Inventory summary">
      ...
      >
        <div class="absolute -right-12 -top-22 h-37.5 w-37.5 rounded-full opacity-55 lg:-right-10.5 lg:-top-21 lg:h-42.5 lg:w-42.5" :class="stat.auraClass" aria-hidden="true"></div>
        ...
        <div class="relative z-10 flex h-7 w-7 flex-none items-center justify-center rounded-[7px] md:h-8 md:w-8 lg:h-8.5 lg:w-8.5" :class="stat.iconClass" aria-hidden="true">
    ...
    <section class="overflow-hidden rounded-lg border border-[#edf0f4] bg-white shadow-[0_12px_28px_rgba(17,24,39,0.04)]">
      ...
      <div class="grid gap-3 p-4 md:hidden">
        ...
        >
          ...
          <dl class="grid grid-cols-2 gap-3 text-xs">
            ...
            <div>
              <dt class="mb-1 font-semibold tracking-[1px] text-[#777d8c]">STOCK LEVEL</dt>
              <dd class="m-0 font-bold text-[#22252c]">
                <span v-if="!isEditing">{{ item.stock }} {{ item.unit }}</span>
                <div v-else class="flex items-center gap-1.5">
                  <input 
                    v-model.number="tempStocks[item.sku]" 
                    type="number" 
                    min="0"
                    class="h-7 w-16 rounded border border-[#e1e5ec] bg-white px-1.5 text-center text-xs font-bold text-[#22252c] outline-none focus:border-[#00449e]"
                  />
                  <span class="text-[11px] font-normal text-[#667085]">{{ item.unit }}</span>
                </div>
              </dd>
            </div>
        ...
        <table class="w-full min-w-170 table-fixed border-collapse md:min-w-full">
          ...
          <thead>
            <tr>
              ...
              <th :class="tableHeadClass" class="text-center">CURRENT STOCK</th>
          ...
          <tbody>
            <tr v-for="item in items" :key="item.sku">
              ...
              <td class="border-b border-[#f0f2f5] px-3 py-3 text-center align-middle text-xs font-bold text-[#22252c] md:px-4 md:py-4 md:text-sm xl:px-5">
                <span v-if="!isEditing">{{ item.stock }}</span>
                <div v-else class="flex justify-center">
                  <input 
                    v-model.number="tempStocks[item.sku]" 
                    type="number" 
                    min="0"
                    class="h-8 w-20 rounded border border-[#e1e5ec] bg-white text-center text-xs font-bold text-[#22252c] outline-none focus:border-[#00449e] focus:ring-1 focus:ring-[#00449e]"
                  />
                </div>
              </td>
      ...
      <footer class="flex flex-col items-start justify-between gap-3.5 px-5.5 py-[18px] min-[821px]:flex-row min-[821px]:items-center">
        <p class="m-0 text-sm text-[#4f5663]">Showing 1 to {{ items.length }} of 1,240 entries</p>
