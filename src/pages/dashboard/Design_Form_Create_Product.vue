...
const submitProduct = async () => {
  ...
  try {
    ...
    // 5. Submit data using Fetch API
    const res = await fetch("http://localhost:8000/api/products", {
      method: "POST",
      body: formData,
      // 'Content-Type' header is automatically set to 'multipart/form-data' by the browser
    })

    if (!res.ok) {
      const errorData = await res.json().catch(() => ({}))
      throw new Error(
        errorData.message || `Failed to create product. Status: ${res.status}`
      )
    }
